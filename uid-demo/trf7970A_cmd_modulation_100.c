...
#define TRF7970A_CMD_MODULATION_MASK_100 0x01

static int trf7970A_cmd_modulation_100_func( int argc, char **argv )
{
  int eno;
  uint8_t read_buf[MULTIIO_READ_BUF_SIZE];
  char    write_buf[strlen("TRF_100 ") + 3 * MULTIIO_WRITE_BUF_SIZE];
  ...
  if( eno == 0 ) {
    ...
    read_buf[1] |= TRF7970A_CMD_MODULATION_MASK_100;
    snprintf( &write_buf[0], sizeof(write_buf), "TRF_100 09 %02"PRIx8, read_buf[1] );
    /* Write to register 0x9 (Modulator and SYS_CLK Control Register)
       Use perameters as previously read, but modulation set to 100% */
    eno = trf7979A_cmd_raw_string(
      &write_buf[0],
      &read_buf[0],
      MULTIIO_READ_BUF_SIZE
...
rtems_shell_cmd_t trf7970A_cmd_modulation_100 = {
  "TRF_100",
  "TRF_100\n"
  "Sets the modulation of the TRF RFID reader to 100%.\n"
  "Returns \"Success\" upon success or \n"
  "\"Failure\" upon failure.\n",
  "TRF",
  trf7970A_cmd_modulation_100_func,
  NULL,
  NULL
};